...
Go 1.25 · [gin](https://github.com/gin-gonic/gin) v1.12 · single binary, no database. Six source files, all `package main`:

| File | Lines | Role |
|---|---|---|
| `main.go` | ~200 | Wiring: config, middleware chain, static routes, block-studio API |
| `auth.go` | ~460 | Accounts (root + users), HMAC sessions, waiting list |
| `gemini.go` | ~190 | REST client for Gemini `generateContent` + output sanitizing |
| `prompts.go` | ~245 | Block registry: one expert system prompt per block type |
| `sites.go` | ~680 | Imported-sites feature: upload, edit, save, domain routing, input validation |
| `build.go` | ~360 | Publish pipeline: local Tailwind compile, PWA generation, GDPR notice |

Tests: `build_test.go`, `auth_test.go`, `sites_test.go` — run with `go test .` (single package; never `./...`, the stray `books/` directory pollutes it).
...
| `GET /gfx/*` `/static/*` | pub | asset directories (`web/gfx`, `web/static`) |
...
| `PUT /api/sites/:id/settings` | site | publication settings `{compile, pwa, gdpr}` + background rebuild |
...
```
sites/<id>/site.json   → { "domain": "example.com", "compile": …, "pwa": …, "gdpr": … }
...
- `PUT /api/sites/:id/settings` `{compile?, pwa?, gdpr?}` — partial update (absent fields untouched), merged into `site.json`, `go BuildSite(id)`. Response echoes the resolved settings.
...
3. **GDPR notice** (`gdpr`): injects a self-contained snippet before `</body>` — « Zéro stress : tout est local », OK button, link to nadaprod.com/legal — shown until dismissed (`localStorage['np-gdpr-ok']`).

The pure rewrite helpers (`extractTailwindConfig`, `rewriteTailwindHTML`, `insertBefore`) are covered by table-driven tests in `build_test.go` (single-line documents, strict vs. mixed configs, CDN tag variants).
...
`bin/build.sh` — downloads the Tailwind v3.4.17 standalone binary to `bin/tailwindcss` if absent (gitignored; failure is non-fatal — local compile just stays off), then `go build -ldflags "-s -w" -o nadaprod .` from the repo root (`full` argument adds `go get -u ./... && go mod tidy`), then `systemctl stop/start/status nadaprod`. The service reads config from real env vars or the `.env` next to the binary's working directory — **`ROOT_PASSWORD` must be in the production `.env` before deploying the auth build** (the server refuses to start without it, and `Restart=always` would loop). Tests: `go test .` (`build_test.go`, `auth_test.go`); see TODO.md for the remaining intended targets.
