...
// build.go — publication d'un site importé.
//
...
const (
	...
	gdprTag = `<script id="__np_gdpr">(function(){try{if(localStorage.getItem('np-gdpr-ok'))return}catch(e){return}
var d=document.createElement('div');d.setAttribute('role','dialog');d.setAttribute('aria-label','Confidentialité');
...
d.innerHTML='<strong style="display:block;margin-bottom:4px">Zéro stress : tout est local.</strong>Ce site ne dépose aucun cookie de suivi et ne collecte aucune donnée personnelle. <a href="https://nadaprod.com/legal" target="_blank" rel="noopener" style="color:#A78BFA">Mentions légales</a><div style="text-align:right;margin-top:10px"><button type="button" style="background:#7C3AED;color:#fff;border:0;border-radius:9px;padding:6px 18px;font:600 13px system-ui,sans-serif;cursor:pointer">OK</button></div>';
...
func (s *SiteStore) buildLock(id string) *sync.Mutex {
...
func (s *SiteStore) flagsFor(id string, meta SiteMeta) buildFlags {
	src := s.srcDir(id)
...
func (s *SiteStore) buildPage(id, rel string, f buildFlags) (bool, error) {
	...
	if f.gdpr {
		out = insertBefore(out, bodyCloseRe, gdprTag)
		changed = true
	}
	if !changed {
		return false, nil
...
func (s *SiteStore) writePWAFiles(id string, meta SiteMeta, f buildFlags) error {
	pub := s.publicDir(id)
	if f.genSW {
...
func (s *SiteStore) BuildSite(id string) {
	mu := s.buildLock(id)
	mu.Lock()
	defer mu.Unlock()
	...
	log.Printf("build %s : %d/%d pages publiées (compile=%t pwa=%t gdpr=%t)", id, built, len(pages), f.compile, f.genSW, f.gdpr)
}

// RebuildPage reconstruit une seule page (après un « Enregistrer »).
func (s *SiteStore) RebuildPage(id, rel string) {
	mu := s.buildLock(id)
	mu.Lock()
	defer mu.Unlock()
